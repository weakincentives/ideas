...
- Should feedback providers be able to *escalate* — convert to a blocking
  gate after repeated ignored warnings — or does that blur the
  advisory/binding line more than it is worth?
- Is there a portable notion of "stop reason" rich enough for gates to treat
  model-initiated stops differently from harness-initiated ones?
- Should completion feedback be structured (missing-item lists) rather than
  prose, so continuation turns can be measured for progress against it?
